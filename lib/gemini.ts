...
// This is per specific instruction for the Gemini API.
...
export async function generateAttendanceSummary(data: AttendanceSummaryData): Promise<AttendanceSummary> {
  if (!process.env.API_KEY) {
    throw new Error("La función de análisis con IA no está disponible. Por favor, configura la clave de API de Gemini.");
  }
